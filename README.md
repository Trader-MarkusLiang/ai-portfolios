...
- 日报素材：写入 `data/wechat_groups/summaries/`，可提交到 GitHub，并被 `python -m src.main` 作为 `微信投资群摘要` 数据源读取。
- 本地索引：写入 `data/wechat_groups/processed/`，默认不提交，避免把逐条消息预览推到 GitHub。
- 首次运行需在 macOS 授权：系统设置 → 隐私与安全性 → 辅助功能，允许 Terminal/Codex/osascript 控制微信。
